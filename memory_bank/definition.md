...
| F-01-01 | 自動メニュー生成 (AI) | 生成AIを用いて練習メニュー案を自動生成する | - 利用する生成AIモデルを以下から選択可:<br> - OpenAI (gpt-4o)<br> - Google (gemini-2.0-flash)<br> - Anthropic (claude-3.5-sonnet)<br>- 各AIサービスのAPIキーはユーザーが入力する形式<br>- システムが適切なプロンプトを生成し、メニューを作成<br>- プロンプトには、負荷レベル、時間、RAGによる類似メニュー情報（時間ベース、コサイン類似度による検索結果）、備考などを含む |
...
| F-02-02 | RAGによる参照 | 関連性の高い過去メニュー情報をAI生成に活用 | - **実装済み**<br>- OpenAI Embeddings APIを使用してメニューのベクトル化を実装<br>- コサイン類似度による類似メニュー検索機能を実装<br>- 類似メニューの検索結果をAIプロンプトに含め、参考情報として提供<br>- 検索結果には類似度スコアを表示（%表示）<br>- 上位5件のメニューを表示<br>- 類似メニュー検索はオプション機能として実装（トグルで有効/無効を切り替え可能）<br>- 検索用のOpenAI APIキーは別途入力欄を用意<br>- 検索結果はAIプロンプトに含められ、参考情報として活用 |
| F-03 | メニュー出力 | 生成されたメニューを出力・ダウンロード | - PDF/CSVに対応<br>- 出力項目: 種目、距離、本数、サークル、休憩、所要時間、備考など<br>- PDF出力の改善（実装済み）:<br>  - 距離と本数の表示を明確化<br>  - 合計距離を各項目に追加<br>  - サイクルタイムの表示を整理<br>  - 所要時間を右寄せで表示<br>  - セクション合計行のデザインを改善<br>  - 総合計行に合計距離を追加 |
| F-04 | CSV出力 | 生成されたメニューをCSV形式で出力 | - UTF-8 BOMを付与して日本語の文字化けを防止（実装済み） |
...
- **プロンプトエンジニアリング**：
  ...
  - 出力形式 (JSON) もプロンプトで指定
  - RAGによる過去メニューの参照機能（コサイン類似度による検索結果を挿入）
