...
| F-02-02 | RAGによる参照 | 関連性の高い過去メニュー情報をAI生成に活用 | - **実装済み**<br>- OpenAI Embeddings APIを使用してメニューのベクトル化を実装<br>- コサイン類似度による類似メニュー検索機能を実装<br>- 類似メニューの検索結果をAIプロンプトに含め、参考情報として提供<br>- 検索結果には類似度スコアを表示（%表示）<br>- 上位5件のメニューを表示<br>- 類似メニュー検索はオプション機能として実装（トグルで有効/無効を切り替え可能）<br>- 検索用のOpenAI APIキーは別途入力欄を用意 |
| F-03 | メニュー出力 | 生成されたメニューを出力・ダウンロード | - PDF/CSVに対応<br>- 出力項目: 種目、距離、本数、サークル、休憩、所要時間、備考など<br>- PDF出力の改善:<br>  - 距離と本数の表示を明確化<br>  - 合計距離を各項目に追加<br>  - サイクルタイムの表示を整理<br>  - 所要時間を右寄せで表示<br>  - セクション合計行のデザインを改善<br>  - 総合計行に合計距離を追加 |
| F-04 | CSV出力 | 生成されたメニューをCSV形式で出力 | - UTF-8 BOMを付与して日本語の文字化けを防止 |
